<template>
  <v-toolbar density="comfortable" class="me-4">
    <v-toolbar-title>
      <AppBackBtn class="me-1" />
      <span class="text-subtitle-2">编辑 {{ editItem.name }}</span>
    </v-toolbar-title>
    <v-spacer />
    <v-btn size="small" class="me-2" variant="tonal" @click="cancel"
      >取消</v-btn
    >
    <v-btn size="small" variant="flat" @click="save" color="indigo" :loading="loading">更新数据</v-btn>
  </v-toolbar>
  <v-divider />
  <v-container>
    <v-row>
...
const save = async () => {
  ...
  loading.value = true;
  const { success } = await pms.brand().update(id, editItem);
  if (!success) {
    useSnackbar("更新品牌失败");
    loading.value = false;
    return;
...
const fetch = async () => {
  const { success, data } = await pms.brand().get(id);
  if (!success) {
    useSnackbar("获取品牌信息失败");
    return;
  }
