  ...
  <AppToolkitBar>
    <v-dialog width="auto">
      ...
      <v-card min-width="400">
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn size="small" color="indigo" @click="onCancelBind">取消</v-btn>
          <v-btn size="small" color="indigo" @click="onBindUser">提交</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-divider class="mx-2 my-3" vertical> </v-divider>
    <v-btn prepend-icon="mdi-cloud-download" size="small" class="border"
      >下载订单</v-btn
    >
    <v-spacer />
    <v-select
  ...
  <v-card flat rounded="0">
      ...
      <v-row no-gutters>
        <v-col cols="12" md="6">
          <v-item v-slot="{ isSelected, toggle }">
            <v-card
              :class="isSelected ? 'active-border' : ''"
              height="100"
              flat
              rounded="0"
              @click="toggle"
            >
              <div class="d-flex align-center justify-center h-100 flex-column">
    ...
    >
      <template v-slot:no-data>
        ...
        <div class="v-card-subtitle text-caption mb-8">快去邀请用户吧</div>
      </template>
    </v-data-table>
    <v-data-table
      v-if="tab === 1"
