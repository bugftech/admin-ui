<template>
                ...
                <template v-slot:item="{ item }">
                  <tr>
                    <td v-for="(header, i) in headers" :key="i">
                      ...
                      <div
                        v-else-if="header.contentType === 'array'"
                        class="d-flex"
                      >
                        <v-chip
  ...
  <v-dialog v-model="editRowDialog" max-width="800">
    <v-card>
      <v-toolbar>
      ...
      <v-card-actions>
        ...
        <v-btn size="small" color="indigo" @click="upsertRow">提交</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
...
const onEditRow = async (item: any) => {
  ...
  for (const key in item) {
    ...
    if (header && header.contentType === "array") {
      obj[key] = item[key].split(",")
    } else {
      obj[key] = item[key];
    }
  }
